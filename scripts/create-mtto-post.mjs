/**
 * Crea el post de Mantenimiento Preventivo en el Laboratorio.
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/marzo/08-mantenimiento-preventivo.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '08-mantenimiento-preventivo.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-mantenimiento-preventivo.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Cómo Implementar un Programa de Mantenimiento Preventivo en tu Laboratorio',
    slug: { _type: 'slug', current: 'como-implementar-programa-mantenimiento-preventivo-laboratorio' },
    ...
    excerpt: 'Guía completa para implementar un programa de mantenimiento preventivo en el laboratorio analítico: frecuencias por equipo, checklists, contratos, indicadores y ROI.',
    body,

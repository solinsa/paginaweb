/**
 * Crea el post de Análisis de Ácidos Grasos en Alimentos por GC-FID.
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/marzo/09-acidos-grasos-gc-fid.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '09-acidos-grasos-gc-fid.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-acidos-grasos-gc-fid.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Análisis de Ácidos Grasos en Alimentos por GC-FID: Perfil Lipídico y Normatividad',
    slug: { _type: 'slug', current: 'analisis-acidos-grasos-alimentos-gc-fid-perfil-lipidico' },
    ...
    excerpt: 'Guía técnica para el análisis de ácidos grasos en alimentos por GC-FID: derivatización FAME, columna DB-23, identificación por tiempo de retención y cumplimiento de la NOM-051.',
    body,

/**
 * Crea y publica el post "Análisis de Cannabinoides por HPLC: Método, Columnas y Detectores Recomendados".
 * Lee el body markdown y lo convierte a Portable Text para Sanity.
 * Imagen: public/abril/07-cannabinoides-hplc.png
 */
...
// Conversión de markdown a Portable Text
function markdownToPortableText(md) {
  ...
  while (i < lines.length) {
    ...
    // Párrafos con soporte para **bold** y *italic*
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/abril/07-cannabinoides-hplc.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '07-cannabinoides-hplc.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-cannabinoides-hplc.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Análisis de Cannabinoides por HPLC: Método, Columnas y Detectores Recomendados',
    slug: { _type: 'slug', current: 'analisis-cannabinoides-hplc-metodo-columnas-detectores' },
    author: 'Equipo Solinsa',
    mainImage: { _type: 'image', asset: { _type: 'reference', _ref: asset._id } },
    category: { _type: 'reference', _ref: 'cat-ind-cannabis' },
    excerpt: 'Determinación de THC, CBD, CBN, CBG por HPLC: fase móvil, columna C18, detector UV/PDA o MS, preparación de muestra, calibración y control de calidad. Normatividad COFEPRIS.',
    body,

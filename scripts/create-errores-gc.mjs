/**
 * Crea y publica: '8 Errores Comunes en el Análisis por Cromatografía de Gases (y Cómo Evitarlos)'
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/mayo/06-errores-gc.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '06-errores-gc.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Categoría
  const catId = 'cat-serv-diag';
  console.log(`   📁 Categoría: ${catId}`);

  // 3. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-errores-gc.md', 'utf-8');
  ...
  // 4. Crear post
  const title = '8 Errores Comunes en el Análisis por Cromatografía de Gases (y Cómo Evitarlos)';
  const slug = '8-errores-comunes-analisis-cromatografia-gases-como-evitarlos';
  ...
  const doc = {
    ...
    excerpt: 'Los 8 errores más frecuentes en cromatografía de gases: inyección incorrecta, contaminación del liner, septum dañado, columna mal instalada, fugas, temperatura incorrecta, dilución inadecuada y mala integración. Soluciones prácticas para cada uno.',
    body,

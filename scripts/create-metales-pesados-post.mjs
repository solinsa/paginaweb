/**
 * Crea el post de Análisis de Metales Pesados en Alimentos por AAS.
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/abril/04-metales-pesados-aas.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '04-metales-pesados-aas.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-metales-pesados-aas.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Análisis de Metales Pesados en Alimentos por AAS: Métodos Oficiales y Límites NOM',
    slug: { _type: 'slug', current: 'analisis-metales-pesados-alimentos-aas-metodos-oficiales-limites-nom' },
    author: 'Equipo Solinsa',
    mainImage: { _type: 'image', asset: { _type: 'reference', _ref: asset._id } },
    category: { _type: 'reference', _ref: 'cat-ind-alimentos' },
    excerpt: 'Métodos oficiales para análisis de metales pesados en alimentos por AAS: digestión ácida, llama para Pb/Cd/Cu/Zn, horno de grafito para As/Hg, límites NOM, CRM y control de calidad.',
    body,

/**
 * Crea el post de Espectrometría ICP-OES para Análisis Elemental en Matrices Ambientales.
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/abril/08-icp-oes-elemental.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '08-icp-oes-elemental.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-icp-oes-ambiental.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Espectrometría ICP-OES: Guía Completa para Análisis Elemental en Matrices Ambientales',
    slug: { _type: 'slug', current: 'espectrometria-icp-oes-guia-completa-analisis-elemental-matrices-ambientales' },
    author: 'Equipo Solinsa',
    mainImage: { _type: 'image', asset: { _type: 'reference', _ref: asset._id } },
    category: { _type: 'reference', _ref: 'cat-icp' },
    excerpt: 'Guía completa de ICP-OES para análisis elemental en matrices ambientales: principios, configuración, líneas de emisión, interferencias espectrales, preparación de muestra, límites de detección y aplicaciones.',
    body,

/**
 * Crea el post "Cómo Elegir un Proveedor de Servicio para tu Equipo de Cromatografía".
...
// Función para convertir markdown a Portable Text
    ...
    if (line.trim() && !line.startsWith('|') && !line.startsWith('---')) {
      ...
      while (remaining.length > 0) {
        ...
        if (strongMatch && emMatch) {
          // **text** (strong) takes priority over *text* (em) when indices are equal
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/marzo/10-proveedor-servicio.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '10-proveedor-servicio.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-proveedor-servicio.md', 'utf-8');
  const body = markdownToPortableText(bodyMd);
  console.log(`📝 Body: ${body.length} bloques generados`);

  // 3. Eliminar post existente si lo hay (para recrearlo)
  const postId = 'S5bzMTDRifCZEnNLwWIYpi';
  const existing = await client.getDocument(postId).catch(() => null);
  if (existing) {
    console.log(`🗑️  Eliminando post existente: ${postId}`);
    await client.delete(postId);
    // Also clean up any draft
    await client.delete(`drafts.${postId}`).catch(() => null);
  ...
  const doc = {
    _type: 'post',
    title: 'Cómo Elegir un Proveedor de Servicio para tu Equipo de Cromatografía',
    slug: { _type: 'slug', current: 'como-elegir-proveedor-servicio-equipo-cromatografia' },
    author: 'Equipo Solinsa',
    mainImage: { _type: 'image', asset: { _type: 'reference', _ref: asset._id } },
    category: { _type: 'reference', _ref: 'cat-serv-mtto' },
    excerpt: 'Guía de selección de proveedor de servicio para equipos de cromatografía: cobertura geográfica, respuesta en sitio, inventario de refacciones, documentación para auditoría y experiencia multimarca.',
    body,

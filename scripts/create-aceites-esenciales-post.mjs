/**
 * Crea el post de Aceites Esenciales: Análisis por GC-MS para Control de Calidad y Autenticidad.
...
function markdownToPortableText(md) {
  ...
  while (i < lines.length) {
    ...
    // Bloque de código (líneas que comienzan con tres backticks o bloque indentado)
    if (line.trim().startsWith('```')) {
      const codeLines = [];
      i++;
      while (i < lines.length && !lines[i].trim().startsWith('```')) {
      ...
      if (codeLines.length > 0) {
        blocks.push({
          _type: 'block',
          style: 'normal',
          children: [{ _type: 'span', marks: ['code'], text: codeLines.join('\n') }],
        });
      }
      continue;
    }
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/mayo/03-aceites-esenciales-gcms.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '03-aceites-esenciales-gcms.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-aceites-esenciales-gcms.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Aceites Esenciales: Análisis por GC-MS para Control de Calidad y Autenticidad',
    slug: { _type: 'slug', current: 'aceites-esenciales-analisis-gcms-control-calidad-autenticidad' },
    ...
    excerpt: 'Análisis de aceites esenciales por GC-MS: perfil de terpenos, identificación por biblioteca NIST, cuantificación por FID, detección de adulteración y control de calidad en laboratorio.',
    body,
  ...
  // 4. Publicar (mover de drafts a published)

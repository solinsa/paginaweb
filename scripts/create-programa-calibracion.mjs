/**
 * Crea el post de cómo Establecer un Programa de Calibración de Equipos para tu Laboratorio.
...
async function main() {
  // 1. Subir imagen
  const imagePath = resolve(import.meta.dirname, '..', 'public/mayo/02-programa-calibracion.png');
  console.log(`📤 Subiendo imagen: ${imagePath}`);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '02-programa-calibracion.png',
  });
  console.log(`✅ Imagen subida: ${asset._id}`);

  // 2. Leer y convertir body
  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-programa-calibracion.md', 'utf-8');
  ...
  const doc = {
    _type: 'post',
    title: 'Cómo Establecer un Programa de Calibración de Equipos para tu Laboratorio',
    slug: { _type: 'slug', current: 'como-establecer-programa-calibracion-equipos-laboratorio' },
    author: 'Equipo Solinsa',
    mainImage: { _type: 'image', asset: { _type: 'reference', _ref: asset._id } },
    category: { _type: 'reference', _ref: 'cat-serv-calib' },
    excerpt: 'Guía completa para establecer un programa de calibración de equipos en tu laboratorio: identificación de equipos críticos, frecuencias, estándares trazables CRM vs SRM, criterios de aceptación y registro documental.',
    body,

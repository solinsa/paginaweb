/**
 * Crea y publica: 'Analisis de Gases de Combustion por GC-TCD: Aplicaciones en la Industria Energetica'
...
async function main() {
  const imagePath = resolve(import.meta.dirname, '..', 'public/mayo/09-gases-combustion-gc-tcd.png');
  console.log('Subiendo imagen: ' + imagePath);
  const asset = await client.assets.upload('image', createReadStream(imagePath), {
    contentType: 'image/png',
    filename: '09-gases-combustion-gc-tcd.png',
  });
  console.log('Imagen subida: ' + asset._id);

  const catId = 'cat-ind-petroquimica';
  console.log('Categoria: ' + catId);

  const bodyMd = readFileSync('/Users/samuelnavarro/Desktop/body-gases-combustion-gc.md', 'utf-8');
  ...
  const title = 'Analisis de Gases de Combustion por GC-TCD: Aplicaciones en la Industria Energetica';
  const slug = 'analisis-gases-combustion-gc-tcd-aplicaciones-industria-energetica';
  ...
  const doc = {
    ...
    excerpt: 'Determinacion de H2, CO, CO2, CH4, O2 y N2 por GC-TCD con columnas de tamiz molecular y Porapak, validas de conmutacion. Aplicaciones en biogas, combustion y refinacion petroquimica.',
    body,
  ...
  if (draft) {
    ...
    console.log('Post ya publicado: ' + cleanId);
  }

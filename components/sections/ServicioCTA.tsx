...
export default function ServicioCTA({ title, description, ctaLabel = 'Solicitar este servicio', ctaWhatsappText }: ServicioCTAProps) {
  const waLink = `${WA}?text=${encodeURIComponent(ctaWhatsappText || `Solicito servicio: ${title}`)}`
    ...
    <section className="bg-primary-dark text-white py-20">
      <div className="max-w-3xl mx-auto px-margin-desktop text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-10">
          ...
          <Link href="/servicio" className="btn btn-ghost on-dark">Ver todos los servicios</Link>
        </div>
      </div>
    </section>
  )
